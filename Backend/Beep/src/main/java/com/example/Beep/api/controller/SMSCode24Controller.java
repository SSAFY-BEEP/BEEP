...
public class SMSCode24Controller {
    ...
    public ApiResult<?> sendCode(@RequestParam String phoneNumber, @RequestParam String code) {
    ...
    @PreAuthorize("hasRole('USER')")
    public ApiResult<?> isCorrect(@RequestParam String phoneNumber, @RequestParam String code) {
        return new ApiResult<>(smsCode24Service.isCorrect(phoneNumber,code), HttpStatus.OK);
    }

    @ApiOperation(value = "문자확인코드 목록 조회", notes = "모든 문자확인코드 목록 조회")
    @GetMapping("/list")
    public ApiResult<?> getList() {
        return new ApiResult<>(smsCode24Repository.findAll(),HttpStatus.OK);
    }
}
