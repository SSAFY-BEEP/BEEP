...
public class Message24Controller {
    ...
    public ApiResult<?> getReceiveMessage(){
        return new ApiResult<List<Message24>>(service.getReceiveMessage(), HttpStatus.OK);
    }
    ...
    public ApiResult<?> getSendMessage(){
        return new ApiResult<List<Message24>>(service.getSendMessage(), HttpStatus.OK);
    }

//    @ApiOperation(value = "메세지 발송/저장", notes = "메세지24 저장(차단시킨 상대일경우 수신에는 저장안함)/차단됐으면 true, 안됐으면 false 리턴")
//    @PostMapping
//    @PreAuthorize("hasRole('USER')")
//    public ApiResult<?> sendMessage(@RequestBody Message24RequestDto.sendMessage message){
//        //receiver가 차단했는지 체크
//        boolean isBlocked = blockService.isBlocked(message.getReceiverNum());
//
//        service.sendMessage(message, isBlocked);
//
//        return new ApiResult<String>(isBlocked? "차단된 메세지" : "메세지 정상 발송", HttpStatus.OK);
//    }
    ...
    @PreAuthorize("hasRole('USER')")
    public ApiResult<?> sendMessageWithFile(@RequestPart(required = false) MultipartFile file ,@RequestPart S3RequestDto.sendMessage24 message24) {
        //S3에 파일 등록
        service.sendMessageWithFile(file, message24);

        return new ApiResult<>(message24.getContent(), HttpStatus.OK);
    }

    @ApiOperation(value = "모든 메세지 목록 조회(테스트용)", notes = "모든 회원의 메세지 조회")
    @GetMapping
    public ApiResult<?> getAllMessage(){
        return new ApiResult<List<Message24>>(service.getAllMessage(), HttpStatus.OK);
    }

    @ApiOperation(value = "id로 메세지 하나 조회", notes = "메세지 id로 메세지 조회")
    @GetMapping("/{id}")
    @PreAuthorize("hasAnyRole('USER','ADMIN')")
    public ApiResult<?> getMessage(@PathVariable String id){
        return new ApiResult<Message24>(service.getMessage(id), HttpStatus.OK);
    }

    @ApiOperation(value = "id로 메세지 삭제", notes = "메세지 id와 해당 데이터 해당 데이터 요청자(토큰)로 메세지 삭제")
    @DeleteMapping
    @PreAuthorize("hasRole('USER')")
    public ApiResult<?> deleteMessageById(@RequestParam String messageId){
        //DB에서 메세지 삭제
    ...
    @PreAuthorize("hasRole('USER')")
    public ApiResult<?> saveMessageById(@PathVariable String messageId){
    ...
    @PreAuthorize("hasRole('USER')")
    public ApiResult<?> blockMessageById(@PathVariable String messageId){
        //대화관계 차단
        ...
        return new ApiResult<String>(result, HttpStatus.OK);
    }
}
